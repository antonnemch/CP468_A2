...
def select_var_mrv(csp: CSP) -> Optional[Var]:
    """
    pick the next cell to fill using MRV (smallest domain).
    if there is a tie, choose the one with most neighbors still unassigned
    """
...
def order_values_lcv(csp: CSP, var: Var) -> List[int]:
    """
    least constraining value heuristic which orders values by how many other domain vlaues they get rid of
    """
